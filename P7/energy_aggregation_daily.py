...
def main():
    ...
    # Cambia el substring a 10 para agrupar por DÍA (YYYY-MM-DD)
    df = df.withColumn("fecha", substring(col("timestamp"), 1, 10))
    
    # Cambia la agregación para usar tus nuevos campos de temperatura/humedad
    daily_df = df.groupBy("fecha", "sensor_id") \
        .agg(
            avg("temperatura").alias("temp_media"),
            avg("humedad").alias("humedad_media")
        ) \
